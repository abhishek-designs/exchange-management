...
type TableProps = {
  headerRows: TableHead[];
  dataRows: Notification[];
  multiSelect: boolean;
};

const AppTable = ({ headerRows, dataRows, multiSelect }: TableProps) => {
  return (
    <IonCard className="main-table notif-table">
          ...
          {headerRows &&
            headerRows.map(row => (
              <IonCard key={row.id} className={`row row-${row.id}`}>
                ...
                {row.isSortable && <IonCard className="fas fa-sort"></IonCard>}
              </IonCard>
            ))}
        </IonCard>
      </IonCard>
      ...
      <IonCard className="table-data">
        ...
        {dataRows &&
          dataRows.map(data => (
            <IonCard key={data.id} className={`data-row data-row-${data.id}`}>
              {/* Notification */}
              ...
              <IonCard className="row row-5">
                <IonText color={data.isAttended ? "primary" : "warning"}>
                  {data.status}
                </IonText>
                {data.isAttended ? (
      ...
      <IonCard className="table-footer">
        <IonCard className="left-footer-content">
          ...
          <IonSelect interface="popover" className="num-option" value="2">
        ...
        <IonCard className="right-footer-content">
          ...
          <IonCard className="pagination-ctrl">
            <IonButton fill="clear" className="page-btn page-back-btn">
              <IonIcon icon={chevronBack} color="primary" />
            </IonButton>
            <IonSelect interface="popover" className="num-option" value="1">
              <IonSelectOption value="1">1</IonSelectOption>
              <IonSelectOption value="2">2</IonSelectOption>
            </IonSelect>
            <IonButton fill="clear" className="page-btn page-forward-btn">
